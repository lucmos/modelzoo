...
from modelzoo.utils.io_utils import load_data
...
def collate_fn(samples: List, split: Split, metadata: MetaData, transform: Callable = None):
...
class MyDataModule(pl.LightningDataModule):
    ...
    def get_anchors(self) -> Dict[str, torch.Tensor]:
        ...
        if self.anchors_mode == AnchorsMode.STRATIFIED_SUBSET:
            shuffled_idxs, shuffled_targets = shuffle(
                ...
                np.asarray(dataset_to_consider["y"]),
                random_state=0,
            )
            ...
            return {
                ...
                "anchors_images": anchors["x"],
                "anchors_targets": anchors["y"],
            ...
            random_images = torch.rand_like(self.extract_batch(dataset_to_consider, [0] * self.anchors_num)["x"])
            return {
                "anchors_idxs": None,
    ...
    def metadata(self) -> MetaData:
        ...
        class_to_idx = self.train_dataset.features["y"]._str2int
        idx_to_class = self.train_dataset.features["y"]._int2str
        ...
        return MetaData(
            ...
            fixed_images=fixed_images["x"],
            fixed_images_targets=fixed_images["y"],
            class_to_idx=class_to_idx,
            idx_to_class=idx_to_class,
            **self.anchors,
        )
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if (stage is None or stage == "fit") and (self.train_dataset is None and self.val_datasets is None):
            traintestdatasets = load_data(self.datasets.hf)
            traintestdatasets.set_format(
                type="torch", columns=[self.datasets.hf.standard_x_key, self.datasets.hf.standard_y_key, "id"]
            )

            anchors_dataset = load_data(self.datasets.anchors)
            anchors_dataset.set_format(
                type="torch", columns=[self.datasets.hf.standard_x_key, self.datasets.hf.standard_y_key, "id"]
            )

            self.anchors_dataset = anchors_dataset["train"]

            self.train_dataset = traintestdatasets["train"]

            self.val_datasets = [traintestdatasets["test"]]
    ...
    def train_dataloader(self) -> DataLoader:
        return DataLoader(
            ...
            pin_memory=True,
            collate_fn=partial(collate_fn, split="train", metadata=self.metadata, transform=self.transform_batch),
        )
    ...
    def val_dataloader(self) -> Sequence[DataLoader]:
        return [
            DataLoader(
                ...
                pin_memory=True,
                collate_fn=partial(collate_fn, split="val", metadata=self.metadata, transform=self.transform_batch),
            )
...
def main(cfg: omegaconf.DictConfig) -> None:
    ...
    # batch_sampler = BatchSampler(RandomSampler(m.train_dataset), batch_size=2048, drop_last=False)
    # dataloader = DataLoader(m.train_dataset, batch_sampler=batch_sampler)
    for x in tqdm(m.train_dataloader()):
        pass
